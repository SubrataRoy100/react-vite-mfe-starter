...
When `demoService` is built, the plugin generates `dist/remoteEntry.js`. This manifest serves as the remote's dynamic runtime index. It exposes:
...
sequenceDiagram
    ...
    Browser->>Remote: Fetch http://localhost:5001/remoteEntry.js
...
`@subrataroy100/mfe-shared` provides an asynchronous, browser-native event bus exported from `@subrataroy100/mfe-shared/events` (and `@subrataroy100/mfe-shared/events/core` for zero-dependency non-React remotes).

### A. Scoped Event Bus Factory (`createMfeEventBus`)
To eliminate event name collisions across teams and remotes, services create a scoped bus instance:
...
// Broadcasts native CustomEvent formatted as 'team-commerce:cart:updated'
checkoutBus.send("cart:updated", { total: 49.99, itemsCount: 3 });
```

Enriched event details dispatch with metadata:
```json
{
  "sender": "checkout-remote",
  "namespace": "team-commerce",
...
### B. React Hook Subscription (`bus.useListener`)
Inside React components, `useListener` manages lifecycle subscriptions with guaranteed unmount cleanup:

```jsx
function CartBadge() {
  const [count, setCount] = useState(0);

  checkoutBus.useListener("cart:updated", (event) => {
    setCount(event.detail.data.itemsCount);
  });
...
### C. Strong Typing via Module Augmentation
Remotes can extend `MfeEventRegistry` to gain full TypeScript autocompletion and payload validation:

```typescript
declare module "@subrataroy100/mfe-shared" {
  interface MfeEventRegistry {
    "team-commerce:cart:updated": { total: number; itemsCount: number };
  }
}
```

### D. Global Broadcast Compatibility
For simple global events, `sendMfeEvent` and `listenMfeEvent` remain supported:
```javascript
import { sendMfeEvent, listenMfeEvent, MFE_EVENTS } from "@subrataroy100/mfe-shared/events";

sendMfeEvent(MFE_EVENTS.PING, { message: "Ping from remote" });
const unsubscribe = listenMfeEvent(MFE_EVENTS.PING, (payload) => console.log(payload));
```
